...
**A useful gut check:** before committing to a cadence, map out the actual hours per session across planning, promotion, delivery, and content repurposing. Multiply by your target frequency. If that number doesn't fit inside your team's actual available capacity, not their ideal capacity, the cadence isn't sustainable and will erode within two to three months. If the founder or CEO is meant to be the face of the session, it's worth separating their actual time commitment from the team's, which is usually far smaller than people assume, as we cover in [how founders turn one hour a month into a full content calendar](/blog/one-hour-a-month-content-calendar).

## The warning signs your current cadence isn't sustainable

A handful of signals show up well before a webinar program actually collapses, and catching them early is easier than rebuilding a program that already stalled. Attendance rate declining three sessions in a row, even as registration numbers hold steady, usually means fatigue is setting in faster than new audience is being added. Promotion time shrinking each month, where the team spends less effort filling the funnel simply because there isn't time, is an early sign the cadence has outpaced actual capacity. And if the person running point on webinars starts describing the next session as something to survive rather than something to plan, that's a team level warning sign no attendance metric will show you in time. Catching any of these early usually means dropping to a slightly lower frequency for a quarter, not abandoning the format altogether.

## When to revisit the decision

A cadence that was right six months ago isn't guaranteed to still be right after a significant change: a funding round that doubles the marketing team, a shift in average deal size, or a big jump in list size from other campaigns. Treat cadence as a decision worth revisiting after any of these, not just on a fixed annual schedule, since the three inputs that determined it originally, audience size, sales cycle, and capacity, can all move independently of each other.
