...
## A simple scoring system to run after every session

Not every attendee deserves the same follow-up, and deciding that by gut feel is slower and less consistent than using a simple point system. A workable version: 3 points for asking a question during the session, 2 points for staying past the halfway mark, 2 points for clicking through on a follow-up link, and 1 point for matching your ICP on title and company size alone. Anyone landing at 5 points or above gets direct sales outreach within 48 hours. Anyone below that stays in the nurture sequence rather than getting a cold "saw you attended our webinar" call that treats a lukewarm signal as a hot one. The exact weighting matters less than having one at all. It turns "who do we follow up with" from a debate into a five minute sort of your attendee list.

## Treat follow-up as part of the event, not an afterthought

The event doesn't end when the call ends. The follow-up sequence is where most of the pipeline conversion actually happens. We cover the exact emails, subject lines, and timing we use in [our guide to webinar follow-up sequences](/blog/webinar-follow-up-email-sequences).
...
There's no universal benchmark, since deal size and audience quality vary too much, but a rough gut check helps: if fewer than one in twenty attendees generates a real sales conversation within a month of the session, the issue is very rarely the offer itself. It's almost always one of the three gaps above, topic fit, session structure, or follow-up. Reviewed against a rolling six session average rather than a single event, that ratio becomes a genuinely useful health check for the whole program, not just one session's performance.
